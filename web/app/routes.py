...
def notification():
    if request.method == 'POST':
        ...
        try:
            ...
            #################################################
            client = ServiceBusClient.from_connection_string(app.config.get('SERVICE_BUS_CONNECTION_STRING'), uamqp_transport=True)
            sender = client.get_queue_sender(app.config.get('SERVICE_BUS_QUEUE_NAME'))
